...
function setup() {
  ...
  createCanvas(max(windowWidth - 10, size * 3), max(windowHeight - 10, 680));
  ...
  keys = [
    ...
    {name:"Landmass", x:25, y:31.1, w:230, h:20, fixed: false},
    {name:"Water", x:25, y:28.6, w:230, h:20, fixed: false},
    {name:"Labels", x:25, y:26.1, w:230, h:20, fixed: false},
    {name:"Title", x:25, y:23.6, w:230, h:20, fixed: false},
    {name:"Eraser", x:50, y:21.1, w:180, h:20, fixed: false},
...
// paint terrain if the draw tools are selected, avoid sliders and raise/lower selection key rectangle
  if (touches.length > 0 &&
      ...
      !touchInRect(offset + 23, bleed * 5.8, 78, 22, false) &&
      !touchInRect(offset + 23, bleed * 12.8, 78, 22, false) &&
      !touchInRect(offset + 163, bleed * 12.8, 78, 22, false) &&
      !touchInRect(offset + 25, bleed * 31.1, 230, 20, false) &&
      !touchInRect(offset + 25, bleed * 28.6, 230, 20, false)) {
...
function touchStarted() {
  ...
  if (touchInRect(offset + 23, bleed * 5.8, 78, 22, false) || 
      touchInRect(offset + 23, bleed * 12.8, 78, 22, false) ||
      touchInRect(offset + 163, bleed * 12.8, 78, 22, false)) {
    return;
  }
...
function drawTreeTwo(x, y) {
  ...
  iconLayer.stroke("#12310C");
  iconLayer.fill("#194111");
  iconLayer.arc(x, y - 2, 10, 10, PI, 0);
  iconLayer.arc(x, y - 2, 10, 10, 0, PI);
...
function createMapKey() {
  ...
  drawTreeTwoPerm(rOffset + 110, bleed * 25.5);
...
function createControls() {
  textAlign(CENTER);
  textSize(27);
  text("Controls", offset + 140, height - bleed * 32.5);
  
  textSize(16);
  text("Raise landmass", offset + 140, height - bleed * 29.5);
  text("Lower landmass", offset + 140, height - bleed * 27);
  text("Add and edit labels", offset + 140, height - bleed * 24.5);
  text("Add and edit title", offset + 140, height - bleed * 22); // 41
  text("Eraser (Icons)", offset + 140, height - bleed * 19.5); // 43.5
  
  textSize(20);
  textAlign(LEFT);
  text("Brush Size", offset, height - bleed * 14);  // 48.5
  text("Sensitivity", rOffset, height - bleed * 14); // 51
  text("Eraser Size", offset, height - bleed * 7); // 51
  ...
  textSize(13);
  text("(Double click)", rOffset + 12, height - bleed * 1.5);
  pop();
}
